...
try:
    ...
    #Remove the last Green_Ration value(Its a Square)

    Green_Ratio = Green_Ratio[:-1]

    #minimun of Green_Ratio is the G value of T circle
    #Sort the Green_Ratio values
    #First value belongs to T circle other two values belong to C and B circle
    Green_Ratio.sort()
    circle_t = Green_Ratio[0]

    circle_c_and_b = sum(Green_Ratio[1:])/2

   """#Enter T Circle Number
    num = int(input('Enter the T circle number\n').strip())
    """
    #Finding Chromium content
    y = circle_c_and_b - circle_t
    x = (y-14.18) / 20.66
    print()
    print('Amount of Chrominum is sample = {:.2f} ppm'.format(x))
        
except:
    print("Upload a clear Picture PLease")
